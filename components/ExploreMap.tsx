...
// group listings by city into explorable places
const PLACES = Array.from(
  LISTINGS.reduce((map, l) => {
    const key = l.city;
    const cur = map.get(key) ?? { city: key, img: l.img, count: 0, min: Infinity };
    cur.count += 1;
    cur.min = Math.min(cur.min, l.price);
    map.set(key, cur);
    return map;
  }, new Map<string, { city: string; img: string; count: number; min: number }>()).values()
);
    ...
    <section className="bg-white px-8 py-24 md:px-14">
      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {PLACES.map((p, i) => (
          ...
          >
            <Link
              href={`/listings?q=${encodeURIComponent(p.city.split(",")[0])}`}
